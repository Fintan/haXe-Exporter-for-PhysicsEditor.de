...
class B2DebugDraw
 {
	...
	//	m_sprite.graphics.lineStyle(m_lineThickness, color.color, m_alpha);
		addLineStyle(m_sprite.graphics,m_lineThickness, color.color, m_alpha);
	...
	//fintan addition----
	// to cater for easel js API differenc eot flash displaylist
//	inline private function addLineStyle(graphics, thickness:Float=1.0, color:Int, alpha:Float):Void{
	inline private function addLineStyle(graphics:#if easeljs easelhx.display.Graphics #else flash.display.Graphics #end, thickness:Float=1.0, color:Int, alpha:Float):Void{
		#if easeljs
		graphics.setStrokeStyle(thickness);
		//graphics.beginStroke("#FF0");
	...
	//inline private function beginFill(graphics, color:Int, alpha:Float):Void{
	inline private function beginFill(graphics:#if easeljs easelhx.display.Graphics #else flash.display.Graphics #end, color:Int, alpha:Float):Void{
		#if easeljs
		//graphics.beginFill("#FF0");
		graphics.beginFill("#"+StringTools.hex(color));
		#else
	...
	public  function DrawSolidPolygon(vertices:Array<B2Vec2>, vertexCount:Int, color:B2Color) : Void{
		...
		beginFill(m_sprite.graphics, color.color, m_fillAlpha);
		for (i in 1...vertexCount)
		{
				m_sprite.graphics.lineTo(vertices[i].x * m_drawScale, vertices[i].y * m_drawScale);
		}
	...
	//	m_sprite.graphics.lineStyle(m_lineThickness, 0xff0000, m_alpha);
		addLineStyle(m_sprite.graphics,m_lineThickness, 0xff0000, m_alpha);
		...
		addLineStyle(m_sprite.graphics,m_lineThickness, 0x00ff00, m_alpha);
		m_sprite.graphics.moveTo(xf.position.x * m_drawScale, xf.position.y * m_drawScale);
